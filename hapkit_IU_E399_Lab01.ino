...
#define SPS 1000  // Samples per second, the frequency of the control loop
...
int MR_max = 972;     // maximum measurement from MR sensor
...
int mode0(float a, float da, float dda) { //Flat
   return 0;
}

int mode1(float a, float da, float dda) { //Wall
    int fo;
    if (a<15){
      fo = 0;
    }
...
int mode2(float a, float da, float dda) { //Valley
  return 0;
}

int mode3(float a, float da, float dda) { //Hillock
  return 0;
}
...
int mode4(float a, float da, float dda) { //Notch
  if( (a < 1.5) && (a > -1.5) ) {
...
int mode5(float a, float da, float dda) {  //Bumpity
  return 0;
}
...
void loop() {
  ...
  if ((micros() - lastHapkitTime) > dt) {
    ...
    thetaC_0 = double(MR_0) * MR2deg;
    thetaCtot = double(MRtot) * MR2deg;
...
void setPwmFrequency(int pin, int divisor) {
  ...
  if (pin == 5) {
    // WGM = 001 ---> Phase corrected, TOP = 255 (sets period)
